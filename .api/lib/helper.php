...
class Helper extends \Prefab
{
     ...
     */
    public function html_js_encode($str, $returnParts = false)
    {
        $str = preg_replace('/^\s+|\n|\r|\s+$/m', '', $str);
        $enc = '';
        ...
        if ($returnParts) {
            return [
                'key'      => $sec,
                'holder'   => '<div id="'.$sec.'"></div>',
                'script'   => '<script type="text/javascript">$(document).ready(function() {var x'.$sec.'x="'.$enc.'";$("#'.$sec.'").html(unescape(x'.$sec.'x));});</script>',
                'noscript' => '<noscript><div id="noscript_notice"><p>Please enable JavaScript!</p></div></noscript>',
            ];
        }
    ...
    public function hashid(
        $salt = 'SHA512ed.f0r.4a5h.sa1tlng',
        $length = 5,
